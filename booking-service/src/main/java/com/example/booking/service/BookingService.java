...
public class BookingService {
    ...
    public Booking createBooking(Long userId, Long roomId, LocalDate start, LocalDate end, String requestId) {
        ...
        String roomBasePath = "/rooms/" + roomId;
        ...
        try {
            callHotel(roomBasePath + "/hold", holdPayload, correlationId).block(timeout);
            callHotel(roomBasePath + "/confirm", Map.of("requestId", requestId), correlationId).block(timeout);
            ...
            try {
                callHotel(roomBasePath + "/release", Map.of("requestId", requestId), correlationId).block(timeout);
            } catch (Exception ignored) {
                // intentionally ignored (best-effort compensation)
            }
    ...
    private Mono<String> callHotel(String path, Map<String, String> payload, String correlationId) {
        return webClient.post()
                ...
                .timeout(timeout)
                .retryWhen(
                        Retry.backoff(retries, Duration.ofMillis(300))
